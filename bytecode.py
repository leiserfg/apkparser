...
#            registers[i] = 0
    ...
    if registers:
        registers_colors = color_range(
            colors["registers_range"][0], colors["registers_range"][1],
            len(registers)
        )
        for i in registers:
    ...
    for DVMBasicMethodBlock in mx.basic_blocks.gets():
        ...
        block_id = hashlib.md5(
            bytearray(sha256 + DVMBasicMethodBlock.get_name(), "UTF-8")
        ).hexdigest()
        ...
        for DVMBasicMethodBlockInstruction in DVMBasicMethodBlock.get_instructions(
            ...
            output = ", ".join(
                mx.get_vm()
                .get_operand_html(i, registers, colors, escape, textwrap.wrap)
                for i in operands
            )
        ...
        if (
            ...
            values.extend(
                DVMBasicMethodBlock.
                get_special_ins(ins_idx - last_instru.get_length()).get_values()
            )
        ...
        for DVMBasicMethodBlockChild in DVMBasicMethodBlock.childs:
            ...
            child_id = hashlib.md5(
                bytearray(
                    sha256 + DVMBasicMethodBlockChild[-1].get_name(), "UTF-8"
                )
            ).hexdigest()
            ...
            for exception_elem in exception_analysis.exceptions:
                ...
                if exception_block:
                    exception_id = hashlib.md5(
                        bytearray(sha256 + exception_block.get_name(), "UTF-8")
                    ).hexdigest()
                    edges_html += "struct_%s:tail -> struct_%s:header  [color=\"%s\", label=\"%s\"];\n" % (
    ...
    for link in new_links:
        ...
        if DVMBasicMethodBlockChild:
            ...
            child_id = hashlib.md5(
                bytearray(
                    sha256 + DVMBasicMethodBlockChild.get_name(), "UTF-8"
                )
            ).hexdigest()
            ...
            if DVMBasicMethodBlock.get_name(
                ...
                for parent in DVMBasicMethodBlock.fathers:
                    ...
                    for idx, child in enumerate(parent[-1].childs):
                        if child[-1
                                ].get_name() == DVMBasicMethodBlock.get_name():
